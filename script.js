...
if (SpeechRecognition) {
    ...
    recognition.onstart = () => {
        ...
        showNotification('Đang lắng nghe...');
    };
    ...
    recognition.onerror = (event) => {
        ...
        showNotification(`Lỗi nhận dạng giọng nói: ${event.error}`);
    };
...
// --- Core API Call ---
async function askCanDee(prompt, fileInfo = null) {
    try {
...
chatForm.addEventListener('submit', async (e) => {
    ...
    if (userFile) {
        filePayload = { name: userFile.name, type: userFile.type, size: userFile.size };
        if (!finalPrompt) {
            finalPrompt = `Hãy phân tích rủi ro của tệp tin này.`;
        }
    }
